...
class SelectUserCard extends StatefulWidget {
  const SelectUserCard({required this.index, required this.users, Key? key})
      : super(key: key);
  final int index;
  final List<UserModel> users;

  @override
  State<SelectUserCard> createState() => _SelectUserCardState();
}

class _SelectUserCardState extends State<SelectUserCard> {
  ...
  Widget build(BuildContext context) {
    return Directionality(
      ...
      child:  GestureDetector(
        ...
        var sss=await  FirebaseServices().addOrder(productModel, 'Manuel', widget.users[widget.index].name, widget.users[widget.index].userId,widget.users[widget.index].nickName);
        if(sss[0]){
